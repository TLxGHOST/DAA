...
public class multistage {
    ...
    public multistage(int vertex) {
        multistage.vertex = vertex;
        cost_ajacency = new int[vertex][vertex];
        d = new int[multistage.vertex];
        cost = new int[multistage.vertex];
    }
    ...
    public void input_matrix(int n) {
        ...
        int temp = 0;
        for (int i = 0; i < n; i++) {
            // here taking input from user and if user enter -1 you move to next vertex
            // and user wants to connect to a specific vertex enter negative value of the
            // node
            // for example if your are entering cost of 1->5 and the next node connection is
            // from 2->6 so you enter -6 and in the next iteration you will be entering
            // values from 2->6
            // and 2-> further nodes
            // and if suppose 2 is only conmnected to 6 then enter -1 and this woill move
            // your
            // current pointerr to 3 ->6 and further on
            for (int j = i; j < n; j++) {
                if (k < -1) {
                    j = Math.abs(k) - 1;
                    temp = j;
                ...
                System.out.println("Enter weight from " + (i + 1) + " Vertex to " + (j + 1) + " vertex");
                k = sc.nextInt();
                if (k < 0) {
                    break;
                } else
